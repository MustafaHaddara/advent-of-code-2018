function goblins_vs_elves(input)
    board = parse_input(input, 3)
    num_elves = count(map(f->f.role=='E', board.fighters))
    ...
    while true
        ...
        println("Trying attack power $elf_attack")
        # println("Round: 0")
        # print_board(board)
        num_rounds = 0
        no_elves_died = true
        while game_is_running(board)
            ...
            n = count(map(f->f.role=='E', board.fighters))
            if n != num_elves
                println("elf died")
                elf_attack += 1
                no_elves_died = false
...
function parse_input(input, elf_attack)
    ...
    for line in input
        ...
        for ch in line
            ...
            if ch == 'G'
                ...
                push!(row, '.')
            elseif ch == 'E'
                push!(fighters, Fighter(x,y, ch, 200, elf_attack))
                push!(row, '.')
            else
                push!(row, ch)
            end
